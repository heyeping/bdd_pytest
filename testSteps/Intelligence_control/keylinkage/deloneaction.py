...
feature = os.path.join(get_pathfile(), "features/controlPage/keylinkage/删除一个动作.feature")

username = ReadConfig().get_login_massage("userName")
password = ReadConfig().get_login_massage("passWord")


@allure.feature("一键执行-删除一个动作")
@scenario(feature, "一键执行-删除一个动作")
...
def to_setting(login_obj, main_obj, intel_obj, myallure_obj, phone_type):
    ...
    time.sleep(1)
    intel_obj.add_device_action(type, index=1)
    time.sleep(1)
    intel_obj.click_save_button(type)
...
@then('删除动作1')
def click_cache1(intel_obj, myallure_obj, phone_type):
    myallure_obj.allure_step('删除动作1')
...
@then('联动延时动作更新成功')
def click_cache1(intel_obj, myallure_obj, phone_type):
    myallure_obj.allure_step('联动延时动作更新成功')

# -*- coding: utf-8 -*-
# @Time    : 2022/3/15 4:04 下午
# @Author  : jinxu
# @File    : lasttimedelay.py
...
feature = os.path.join(get_pathfile(), "features/controlPage/keylinkage/延时为最后一个动作.feature")

username = ReadConfig().get_login_massage("userName")
password = ReadConfig().get_login_massage("passWord")


@allure.feature("一键执行-延时为最后一个动作")
@scenario(feature, "一键执行-延时为最后一个动作")
...
@then('延时拖拽为最后一个动作')
def click_cache1(intel_obj, myallure_obj, phone_type):
    myallure_obj.allure_step('延时拖拽为最后一个动作')
...
def click_cache1(intel_obj, myallure_obj, phone_type):
    myallure_obj.allure_step('toast提示"延时后必须添加一个设备 类型的动作"')
